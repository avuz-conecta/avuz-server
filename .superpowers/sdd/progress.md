...
## POST-DEPLOY BUG (user-reported) — folder share modal never closes — FIXED + VERIFIED LIVE
Root-caused LIVE via javascript_tool on staging (inspected NcDialog props): NcDialog is controlled by `open` (.sync), NOT `show`. FolderSharingModal passed static `:show="true"` (ignored prop) + relied on parent v-if; the dialog's `open` sat at default-true and the ✕/backdrop close never propagated → "never closes / reopens". BoardCloneModal/BoardExportModal share the same latent bug (dismissed via footer buttons, not ✕) — OUT OF SCOPE, follow-up candidate.
FIX (e1ed87c39): `:open.sync="open"` + reactive `open` data + watch(open→emit close). jest 10/10 incl regression test.
